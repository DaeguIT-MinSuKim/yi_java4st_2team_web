...
SELECT to_char(WRITE_DATE, 'hh24') AS WRITE_hour, COUNT(*) AS HOUR_count FROM longrent GROUP BY TO_CHAR(WRITE_DATE, 'hh24') ORDER BY WRITE_hour asc;

---블랙
SELECT * FROM MEMBER;

UPDATE MEMBER SET IS_BLACK='Y' WHERE NAME='홍자승';
UPDATE MEMBER SET IS_BLACK='Y' WHERE NAME='김자승';
UPDATE MEMBER SET IS_BLACK='Y' WHERE NAME='박자승';
SELECT * FROM MEMBER WHERE IS_BLACK = 'Y' ORDER BY name;


SELECT count(*) AS black_y FROM MEMBER WHERE IS_BLACK ='y';
SELECT count(*) AS black_n FROM MEMBER WHERE IS_BLACK ='N';

SELECT 
decode(is_black,'y','블랙리스트','N','일반회원')
FROM MEMBER;

SELECT COUNT(IS_BLACK)
FROM MEMBER;

SELECT is_Black,count(*), ROUND(RATIO_TO_REPORT(COUNT(*)) OVER () *100) AS BLACK_PER
FROM MEMBER
GROUP BY IS_BLACK 
HAVING IS_black in('Y', 'N');

SELECT is_Black,count(*)
FROM MEMBER
GROUP BY IS_BLACK 
HAVING IS_black in('N', 'Y')
ORDER BY IS_BLACK asc;
