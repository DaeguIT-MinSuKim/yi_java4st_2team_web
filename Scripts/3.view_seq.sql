...
DROP SEQUENCE BRAND_CODE_SEQ;
DROP SEQUENCE KIND_CODE_SEQ;
...
--DROP SEQUENCE NOTICE_NO_SEQ;
--DROP TRIGGER TRI_NOTICE_NO_SEQ;
--notice(no)
CREATE SEQUENCE notice_NO_SEQ
	START WITH 1
	INCREMENT BY 1;
	
CREATE OR REPLACE TRIGGER TRI_notice_NO_SEQ
BEFORE INSERT ON notice
FOR EACH ROW
BEGIN
	IF Inserting AND :NEW.NO IS NULL THEN
		SELECT notice_NO_SEQ.NEXTVAL INTO :NEW.NO FROM DUAL;
	END IF;
END;
...
-- KIND
CREATE SEQUENCE KIND_CODE_SEQ
	START WITH 1
	INCREMENT BY 1;

CREATE OR REPLACE TRIGGER KIND_CODE_SEQ
BEFORE INSERT ON KIND
FOR EACH ROW
BEGIN
	IF Inserting AND :NEW.KIND_CODE IS NULL THEN 
		SELECT KIND_CODE_SEQ.NEXTVAL INTO :NEW.KIND_CODE FROM DUAL;
	END IF;
END;

-- BRAND
CREATE SEQUENCE BRAND_CODE_SEQ
	START WITH 1
	INCREMENT BY 1;

CREATE OR REPLACE TRIGGER BRAND_CODE_SEQ
BEFORE INSERT ON BRAND
FOR EACH ROW
BEGIN
	IF Inserting AND :NEW.BRAND_CODE IS NULL THEN 
		SELECT BRAND_CODE_SEQ.NEXTVAL INTO :NEW.BRAND_CODE FROM DUAL;
	END IF;
END;